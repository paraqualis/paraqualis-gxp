...
1. **Create the PAT** at
   ...
   - Token name: `paraqualis-skills-traffic`
   - Expiration: 1 year (or whatever cadence works for you)
   - Repository access → **Only select repositories** → `paraqualis/paraqualis-skills`
   - Permissions → Repository permissions → **Administration: Read-only**
     *(Leave everything else as "No access".)*
   - Click **Generate token**, copy the value (shown only once).
2. **Add the secret** at
   <https://github.com/paraqualis/paraqualis-skills/settings/secrets/actions>
